...
function buildWhatsAppUrl(items: ReturnType<typeof useCartStore.getState>["items"], total: number, giftBox: boolean) {
  ...
  if (giftBox) {
    lines.push(`Gift Box:  Rs ${giftBoxPrice}`);
  }
...
export function CartDrawer() {
  ...
  const [showGiftBoxImage, setShowGiftBoxImage] = useState(false);

  const total = totalPrice();
  const count = totalItems();
  const giftBoxPrice = giftBox ? 300 : 0;
  const grandTotal = total + 250 + giftBoxPrice;
  ...
  return (
    <>
      ...
      >
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-border space-y-3">
            ...
            {/* Gift box option */}
            <label className="flex items-center gap-3 p-3 rounded-2xl border border-border hover:bg-secondary/30 transition cursor-pointer">
              <input
                type="checkbox"
                checked={giftBox}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3">
                  <img
                    src={giftBagImage}
                    alt="Gift box"
                    className="h-12 w-12 rounded-lg object-cover shrink-0 cursor-pointer hover:opacity-80 transition"
                    onClick={(e) => {
                      e.preventDefault();
                      setShowGiftBoxImage(true);
                    }}
                  />
            ...
            {giftBox && (
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">Gift box</span>
                <span className="text-foreground">Rs {giftBoxPrice}</span>
              </div>
            )}
            ...
            <p className="text-xs text-muted-foreground">
              ...
              <a
                href={buildWhatsAppUrl(items, total, giftBox)}
                target="_blank"
      ...
      {/* Gift Box Image Dialog */}
      <Dialog open={showGiftBoxImage} onOpenChange={setShowGiftBoxImage}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden [&>button]:text-white [&>button]:bg-black/50 [&>button]:hover:bg-black/70">
          <img
            src={giftBagImage}
            alt="Gift box"
